...
import time
import log_util
import segment_classification

COMPACT_TIME_SEGMENT=10
...
def compact_log_segment(log_segment):
    ...
    for log in log_segment:
        ...
        if(log[0]==prev_log_entry and (int(log[1])-prev_log_entry_time)<COMPACT_TIME_SEGMENT):
    ...
    for log in log_list:
            ...
            try:
                if not (curr_log_entry[0]=='Document Changed' and curr_log_entry[0]==prev_log_entry[0])\
                        or curr_log_entry_time-prev_log_entry_time>COMPACT_TIME_SEGMENT:
                    compact_log_list.append(log)
...
def compact_file_entry_exit(log_list):
    ...
    for log_index,log in enumerate(log_list[2:]):
        ...
        if curr_log_entry_time_1-prev_log_entry_time > COMPACT_TIME_SEGMENT :
            compact_log_list.append(log)
            ...
            compact_log_list.append((prev_log_entry,curr_log_entry_time_1))
...
def compact_tool_entry_exit(log_list):
    ...
    for log_index,log in enumerate(log_list[2:]):
        ...
        if curr_log_entry_time_1-prev_log_entry_time > COMPACT_TIME_SEGMENT :
            compact_log_list.append(log)
        elif not(curr_log_entry_1[0] == 'Focus Lost Tool'
                 and curr_log_entry_2[0] == "Focus Gained Tool"
                 and prev_log_entry[0]=="Focus Lost Tool"):
            compact_log_list.append(log)
        elif curr_log_entry_1[1]!=curr_log_entry_2[1] or curr_log_entry_2[1]!=prev_log_entry[1] :
            compact_log_list.append(log)
        else:
...
def log_analysis(log_list,user_name):
    ...
    log_list_compact=[compact_tool_entry_exit(log_segment) for log_segment in log_list_compact]
    ...
    log_list_compact = filter(lambda x : len(x)>0 , log_list_compact)

    user_activity=segment_classification.classify_log_segment_list(log_list_compact,user_name)
